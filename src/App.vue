<template>
  ...
  <nav class="sticky top-0 bg-gray-300 border-gray-200 px-2 sm:px-4 py-2.5 z-40">
    ...
    <div class="flex md:order-2 hover:cursor-pointer">
      <HeartIcon @click="goToSaved" class="mr-2 w-7 h-7" :class="[route.name === 'Saved' ? 'text-blue-700' : '']"/>
    ...
    <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-cta">
      <ul class="flex flex-col p-4 mt-4 border border-gray-100 rounded-lg md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 bg-gray-300 dark:border-gray-700">
        <li v-for="item in navItems" :key="item.id" @click="getProducts(item)">
          <a href="#" class="block py-2 pl-3 pr-4 text-gray-700 rounded md:bg-transparent md:p-0" :class="[item.category === route.params.category ? 'text-blue-700' : '']" aria-current="page">{{ item.name }}</a>
        </li>
      </ul>
    </div>
    </div>
  ...
  <CartEmpty @closeEmptyModal="closeEmptyModal" :isEmptyOpen="isEmptyOpen"/>
